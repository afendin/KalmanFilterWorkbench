...
void StateManager::update(float measured_distance, int time)
{
    ...
    float v = (measured_distance - m_predicted_distance) / t;

    // State Update Equation for position
    float estimated_distance = m_predicted_distance +
                               m_trust_distance *
                               (measured_distance - m_predicted_distance);
    set_estimated_distance(estimated_distance);

    // State Update Equation for velocity
    float estimated_velocity = m_predicted_velocity +
                               m_trust_velocity * v;
